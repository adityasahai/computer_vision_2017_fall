...
function [bboxes, confidences, image_ids] = .... 
    run_detector(test_scn_path, w, b, feature_params, feature_size)
...
%   scale_factor = 0.99;
  ...
  threshold = 0.0;
  win = feature_params.template_size / ...
  ...
  for i = 1:length(test_scenes)
      ...
      for s = 1.2:-0.04:0.1   
        img_prime = imresize(img, s);
        if (size(img_prime, 1) < feature_params.template_size ||...
        ...
        for k = 1:rows 
          for j = 1:cols
            feature_set = hog(k:k+win-1, j:j+win-1,:);
            features = reshape(feature_set, [feature_size, 1]);
            confidence = w'*features + b;
            if (confidence > threshold)
              y_min = floor((k-1) * feature_params.hog_cell_size + 1)/s;
              y_max = y_min + feature_params.template_size/s;
              x_min = floor((j-1) * feature_params.hog_cell_size + 1)/s;
              x_max = x_min + feature_params.template_size/s;
              
              cur_bbox = [x_min, y_min, x_max, y_max];
      ...
      [is_maximum] = non_max_supr_bbox(cur_bboxes, cur_confidences,...
                                        size(img));
