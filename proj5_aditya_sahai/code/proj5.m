...
positive_featrues_rerun = false;
if ~exist('features_pos.mat', 'file') || positive_featrues_rerun
  fprintf('Extracting Positive features\n')
  features_pos = get_positive_features( train_path_pos, feature_params );
  save('features_pos.mat', 'features_pos')
else
  load('features_pos.mat')
end
negative_features_rerun = true;
num_negative_examples = 20000;
if ~exist('features_neg.mat', 'file') || negative_features_rerun
  fprintf('Extracting Negative Features\n')
  tic;
  features_neg = get_random_negative_features2( non_face_scn_path,...
...
% % trianing data.
calculate_hard_negatives = false;
if calculate_hard_negatives
  fprintf("Extracting Hard Negatives\n");
  max_num_negatives = 10000;
  threshold = 0.1;
  hard_neg_features = run_hard_negatives_detector(non_face_scn_path, w, b,...
                          feature_params, feature_size, max_num_negatives,...
                          threshold);
  fprintf("Extracted %d hard negatives\n", size(hard_neg_features, 1));
  fprintf("Re-Training SVM Classifier\n");
  X = [X; hard_neg_features];
  Y = [Y; -1*ones(size(hard_neg_features,1),1)];
  [w, b] = vl_svmtrain(X', Y, lambda);

  fprintf('Retrained classifier performance\n')
  confidences = X*w + b;
  label_vector = Y;
...
[bboxes, confidences, image_ids] = run_detector(test_scn_path, w, b,...
                                    feature_params, feature_size);
