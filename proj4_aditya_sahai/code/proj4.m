...
% CLASSIFIER = 'primal svm';
% CLASSIFIER = 'libsvm';
% CLASSIFIER = 'primal svm linear';
...
switch lower(FEATURE)    
    ...
    case 'bag of sift + gist'
        % Accuracy with Bag of Sift + Gist - 0.759
        if ~exist('vocab_2000.mat', 'file')
            ...
            save('vocab_2000.mat', 'vocab')
        end
        
        % SIFT features
        if ~exist('train_image_feats_fast_step_3_2000.mat', 'file')
          train_image_feats = get_bags_of_sifts(train_image_paths);
          save('train_image_feats_fast_step_3_2000.mat', 'train_image_feats')
        else
          load('train_image_feats_fast_step_3_2000.mat')
        end
        
        if ~exist('test_image_feats_fast_step_3_2000.mat', 'file')
          test_image_feats  = get_bags_of_sifts(test_image_paths);
          save('test_image_feats_fast_step_3_2000.mat', 'test_image_feats')
        else
          load('test_image_feats_fast_step_3_2000.mat')
        end
        
        % GIST Features
        if ~exist('train_image_gist_feats.mat', 'file')
          train_image_gist_feats = get_gist(train_image_paths);
          save('train_image_gist_feats.mat', 'train_image_gist_feats');
        else
          load('train_image_gist_feats.mat');
        end
        if ~exist('test_image_gist_feats.mat', 'file')
          test_image_gist_feats = get_gist(test_image_paths);
          save('test_image_gist_feats.mat', 'test_image_gist_feats');
        else
          load('test_image_gist_feats.mat');
        end
    ...
    case 'gist'
      ...
      train_image_feats = train_image_gist_feats;
      test_image_feats = test_image_gist_feats;
...
switch lower(CLASSIFIER)    
    ...
    case 'primal svm'
        predicted_categories = primal_svm_classify(train_image_feats, train_labels, test_image_feats);
        
    case 'libsvm'
        predicted_categories = libsvm_classify(train_image_feats, train_labels, test_image_feats);

    case 'primal svm linear'
        predicted_categories = primal_svm_classify_linear(train_image_feats, train_labels, test_image_feats);
