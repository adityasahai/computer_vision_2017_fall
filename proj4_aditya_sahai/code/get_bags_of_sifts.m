...
%   image_feats2 = zeros(size(image_paths,1), size(vocab, 2));
  kdtree = vl_kdtreebuild(vocab);
tic
  for i = 1:size(image_paths, 1)
    image = imread(image_paths{i, 1});
    image = im2single(image);
    [~, features] = vl_dsift(image, 'step', 3);
    [indices, ~] = vl_kdtreequery(kdtree, vocab, single(features));
    % Consider using histc
...
%       [~, k] = min(d(j, :));
%       image_feats2(i, k) = image_feats2(i, k) + 1;
%     end
  end
toc
  % Normalize histogram
  image_feats = image_feats ./ norm(image_feats);
%   image_feats2 = image_feats2 ./ norm(image_feats2);
end
