...
% forms of spatial verification of matches.
    ...
    confidences = zeros(size(features1, 1), 1);
    matches = zeros(size(features1, 1), 2);
    
    for i = 1:size(features1, 1)
        nd = zeros(1, size(features2, 1));
        for j = 1:size(features2, 1)
            nd(j) = euclidean_distance(features1(i, :), features2(j, :), size(features1, 2));
        end
        [~, k] = sort(nd, 'ascend');
        if nd(2) == 0
            confidences(i) = 0;
        else
            confidences(i) = 1 - nd(k(1))/nd(k(2));
        end
        matches(i, :) = [i, k(1)];
    end
...
function distance = euclidean_distance(element1, element2, length)
    distance = 0;
    for i = 1:length
        distance = distance + (element1(i) - element2(i))^2;
    end
